...
pub const RenderResources = struct {
    ...
    fn init(gpa: std.mem.Allocator, vma: Vma, physical_device: PhysicalDevice, device: Device, layout: descriptor.Layout) !@This() {
        ...
        const default_material: Material = try .init(
            gpa,
            RenderResources.default_material_name,
            device,
            vma,
            set_size,
        ...
        try materials.put(gpa, try gpa.dupe(u8, RenderResources.default_material_name), default_material);
    ...
    pub fn getMeshPtr(self: *@This(), name_id: ?[]const u8) !*Mesh {
        ...
        if (self.meshes.getPtr(RenderResources.default_mesh_name)) |default_mesh| return default_mesh else {
    ...
    pub fn getMaterialPtr(self: *@This(), name_id: ?[]const u8) !*Material {
        ...
        if (self.materials.getPtr(RenderResources.default_material_name)) |default_material| return default_material else {
